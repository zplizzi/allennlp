...
def _read_pretrained_tokens(embeddings_file_uri: str) -> Set[str]:
    ...
    with EmbeddingsTextFile(embeddings_file_uri) as embeddings_file:
        for line_number, line in enumerate(
                Tqdm.tqdm(embeddings_file), start=1):
            token_end = line.find(' ')
            if token_end >= 0:
                ...
                logger.warning(f'Skipping line number %d: %s', line_number,
                               line_begin)
    return tokens
...
class Vocabulary(Registrable):
    ...
    def save_to_files(self, directory: str) -> None:
        ...
        with codecs.open(
                os.path.join(directory, NAMESPACE_PADDING_FILE), 'w',
        ...
        for namespace, mapping in self._index_to_token.items():
            ...
            with codecs.open(
                    os.path.join(directory, namespace + '.txt'), 'w',
                    'utf-8') as token_file:
    ...
    def from_files(cls, directory: str) -> 'Vocabulary':
        ...
        with codecs.open(
                os.path.join(directory, NAMESPACE_PADDING_FILE), 'r',
                'utf-8') as namespace_file:
    ...
    def set_from_file(self,
        ...
        if is_padded:
            assert self._oov_token in self._token_to_index[
                namespace], "OOV token not found!"
    ...
    def from_params(
        ...
        if vocab_type is not None:
            return cls.by_name(vocab_type).from_params(
                params=params, instances=instances)
    ...
    def add_token_to_namespace(self, token: str,
                               namespace: str = 'tokens') -> int:
        """
    ...
    def get_index_to_token_vocabulary(
            self, namespace: str = 'tokens') -> Dict[int, str]:
        return self._index_to_token[namespace]

    def get_token_to_index_vocabulary(
            self, namespace: str = 'tokens') -> Dict[str, int]:
        ...
        if self._retained_counter:
            ...
            for namespace in self._retained_counter:
                tokens_with_counts = list(
                    self._retained_counter[namespace].items())
